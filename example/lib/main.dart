...
void main() {
  runApp(const DiceBearExampleApp());
}

class DiceBearExampleApp extends StatelessWidget {
  const DiceBearExampleApp({super.key});
...
class _DiceBearPlaygroundPageState extends State<DiceBearPlaygroundPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final controlPanel = _ControlCard(
      child: Column(
    ...
    final outputPanel = _ControlCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Text(
          ...
          if (_rawResponse != null)
            _CodeBox(text: _rawResponse!, maxHeight: 260),
        ],
      ),
    );
...
class _CodeBox extends StatelessWidget {
  const _CodeBox({required this.text, this.maxHeight = 220});
